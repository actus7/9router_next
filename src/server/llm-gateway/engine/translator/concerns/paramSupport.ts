...
import type { StripRule } from "./openaiTypes";
...
function clampNumber(body: Record<string, unknown>, key: string, ceiling: number): void {
  if (typeof body[key] === "number" && Number.isFinite(body[key]) && (body[key] as number) > ceiling) {
    body[key] = ceiling;
  }
}
    ...
    if (rule.flattenContent && Array.isArray(body.messages)) {
      for (const msg of body.messages as Record<string, unknown>[]) {
        if (msg && Array.isArray(msg.content)) {
          msg.content = (msg.content as Record<string, unknown>[])
            .map((b: Record<string, unknown>) => (b?.type === "text" && typeof b.text === "string") ? b.text : "")
            .join("");
        }
      }
    }
