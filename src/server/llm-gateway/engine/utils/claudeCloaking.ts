...
import { CLAUDE_TOOL_SUFFIX, CC_DEFAULT_TOOLS } from "../config/appConstants";
...
function generateBillingHeader(payload: Record<string, unknown>) {
...
function deriveUuid(seed: string) {
...
function generateFakeUserID(sessionId: string | undefined, apiKey: string | undefined) {
...
export function cloakClaudeTools(body: Record<string, unknown>) {
  ...
  for (const tool of tools) {
    ...
    const suffixed = suffix(tool.name as string);
    toolNameMap.set(suffixed, tool.name as string);
    clientToolNames.add(tool.name as string);
  ...
  const renamedMessages = messages?.map((msg: Record<string, unknown>) => {
    if (!Array.isArray(msg.content)) return msg;
    const renamedContent = (msg.content as Record<string, unknown>[]).map((block: Record<string, unknown>) =>
      block.type === "tool_use" ? { ...block, name: suffix(block.name as string) } : block
    );
    return { ...msg, content: renamedContent };
  });

  const cloakedBody: Record<string, unknown> = { ...body, tools: allTools, messages: renamedMessages || body.messages };
  ...
  if (
    ...
    clientToolNames.has(toolChoice.name as string)
  ) {
    cloakedBody.tool_choice = { ...toolChoice, name: suffix(toolChoice.name as string) };
  }
...
export function decloakToolNames(body: Record<string, unknown>, toolNameMap: Map<string, string>) {
  if (!toolNameMap?.size || !Array.isArray(body?.content)) return body;
  const content = (body.content as Record<string, unknown>[]).map((block: Record<string, unknown>) => {
    if (block?.type === "tool_use" && toolNameMap.has(block.name as string)) {
      return { ...block, name: toolNameMap.get(block.name as string) };
    }
    return block;
  });
  return { ...body, content };
 ...
 */
export function applyCloaking(body: Record<string, unknown>, apiKey: string, sessionId?: string) {
  ...
  if (!existingUserId) {
    result.metadata = { ...(metadata || {}), user_id: generateFakeUserID(sessionId, apiKey) };
  }
