...
// Batch tests can hit dozens of connections; running them one at a time makes
// a single POST take minutes and risks a platform/proxy timeout with no
// progress feedback. Bound concurrency instead of firing them all in
// parallel (which would hammer every upstream provider at once).
const TEST_CONCURRENCY_LIMIT = 5;

async function testConnectionsWithLimit(
  connections: Record<string, unknown>[],
  getAuthGroupFn: (providerId: string, connection: Record<string, unknown>) => string,
  limit: number,
): Promise<Array<Record<string, unknown>>> {
  const results: Array<Record<string, unknown>> = new Array(connections.length);
  let nextIndex = 0;
    ...
    for (;;) {
      ...
      try {
        ...
        results[i] = {
          ...
          authType: conn.authType || getAuthGroupFn(conn.provider as string, conn),
          valid: data.ok,
  ...
  await Promise.all(Array.from({ length: Math.min(limit, connections.length) }, worker));
  return results;
}
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    const testableConnections = connectionsToTest.filter((conn) => !noAuthProviders.has(conn.provider as string));
    const results: Array<Record<string, unknown>> = await testConnectionsWithLimit(testableConnections, getAuthGroup, TEST_CONCURRENCY_LIMIT);
      ...
      for (const [pId, pInfo] of Object.entries(allFreeProviders)) {
        ...
        if (pInfo.noAuth) {
          ...
          results.push({
            ...
            diagnosis: testResult.ok ? null : { type: "connectivity", source: "upstream" },
            statusCode: null,
            testedAt: new Date().toISOString(),
          });
