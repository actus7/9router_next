...
const fetchWithTimeout = (
  ...
  timeout = 10000,
): Promise<Response> => safePublicFetch(url, { ...options, destinationPolicy, timeoutMs: timeout });
...
const getErrorMessage = (error: Error & { cause?: { code?: string } }): string => {
...
const getModelsErrorMessage = (status: number): string => {
...
const getChatErrorMessage = (status: number): string => {
  if (status === 401 || status === 403) return "API key unauthorized";
...
// Validate a provider-node API key against its configured base URL.
export async function POST(request: NextRequest): Promise<NextResponse> {
  try {
    const body = await request.json();
    const { baseUrl, apiKey, type, modelId } = body;
    const destinationPolicy: DestinationPolicy = isLocalRequest(request) ? "trusted-local" : "public-only";
    ...
    if (destinationPolicy === "public-only") {
      try {
    ...
    const errorMessage = getErrorMessage(error as Error & { cause?: { code?: string } });
    console.error("Error validating provider node:", {
      message: (error as Error).message,
      cause: (error as Error & { cause?: { code?: string } }).cause,
      code: (error as Error & { cause?: { code?: string } }).cause?.code,
