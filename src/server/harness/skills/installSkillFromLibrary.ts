...
async function discoverSkillMarkdownViaGithub(
  ...
  for (const branch of ["main", "master"] as const) {
    ...
    try {
      ...
      listing = (await response.json()) as GitHubContentEntry[];
    } catch {
      continue;
    }
    ...
    const ordered = [...preferred, ...folders.filter((entry) => !preferred.includes(entry))];
...
export async function installSkillFromLibrary(options: {
  ...
  if (!raw) {
    const discovered = await discoverSkillMarkdownViaGithub(source, skillId);
    if (discovered) {
      raw = discovered.raw;
