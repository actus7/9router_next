import { AMAZON_Q_CONFIG, assertValidAwsRegion } from "../constants/oauth";
...
const amazonQ = {
  config: AMAZON_Q_CONFIG as ProviderConfig,
  ...
  requestDeviceCode: async (config: ProviderConfig, codeChallenge: string, options: Record<string, unknown> = {}): Promise<DeviceCodeResponse> => {
    ...
    if (!registerRes.ok) {
      ...
      throw new Error(`Amazon Q client registration failed: ${error}`);
    }
    ...
    if (!deviceRes.ok) {
      ...
      throw new Error(`Amazon Q device authorization failed: ${error}`);
    }
  ...
  mapTokens: (tokens: Record<string, unknown>): MappedTokens => {
    ...
    const mapped: MappedTokens = {
      ...
      providerSpecificData: {
        ...
        startUrl: (tokens._startUrl as string) || (AMAZON_Q_CONFIG as ProviderConfig).startUrl,
      },
    };
    return mapped;
  },
