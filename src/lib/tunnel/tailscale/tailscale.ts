...
import { installTailscaleForCurrentPlatform } from "./tailscaleInstall";
import { startFunnelForRuntime, type FunnelResult } from "./tailscaleFunnel";
...
export async function installTailscale(sudoPassword: string, hostname: string, onProgress?: (msg: string) => void): Promise<{ success: boolean; authUrl?: string; alreadyLoggedIn?: boolean }> {
  ...
  const installedOnWindows = await installTailscaleForCurrentPlatform(sudoPassword, log);
  if (installedOnWindows) return { success: true };
...
export async function startFunnel(port: number): Promise<FunnelResult> {
  ...
  return startFunnelForRuntime(port, {
    bin,
    socketFlags: SOCKET_FLAG,
    args: tsArgs("funnel", "--bg", `${port}`),
    getActualUrl: getActualFunnelUrl,
    getFallbackUrl: () => getTailscaleFunnelUrl(port),
  });
}
