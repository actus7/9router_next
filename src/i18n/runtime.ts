...
function getLocaleFromCookie(): Locale {
  ...
  const value: string = cookie ? decodeURIComponent(cookie.split("=")[1]) : DEFAULT_LOCALE;
...
export function translate(text: string | null | undefined): string | null | undefined {
...
export function onLocaleChange(callback: ReloadCallback): () => void {
  ...
  return () => {
    reloadCallbacks = reloadCallbacks.filter((cb: ReloadCallback) => cb !== callback);
  };
}
...
function processTextNode(node: Text): void {
  ...
  if (!translatedNode._originalText) {
