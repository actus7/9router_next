...
interface ManagedMcpServer {
  ...
  transport: "http" | "sse";
  oauth?: boolean;
  toolPolicy?: Record<string, "allow">;
}
...
const LOCAL_STDIO_PLUGINS: LocalStdioPlugin[] = [
  {
    name: "browsermcp",
    title: "Browser MCP",
...
function buildManagedMcpServers(plugins: RemotePlugin[] | unknown): ManagedMcpServer[] {
  const list = Array.isArray(plugins) ? plugins : [];
  const out: ManagedMcpServer[] = [];
  const seen = new Set<string>();
  for (const p of list) {
    const plugin = p as RemotePlugin;
    if (!plugin?.name || !plugin?.url || seen.has(plugin.name)) continue;
    seen.add(plugin.name);
    const entry: ManagedMcpServer = {
      name: plugin.name,
      url: plugin.url,
      transport: plugin.transport || (/\/sse(\b|\/)/i.test(plugin.url) ? "sse" : "http"),
    };
    if (plugin.oauth) entry.oauth = true;
    if (Array.isArray(plugin.toolNames) && plugin.toolNames.length > 0) {
      ...
      for (const raw of plugin.toolNames) {
        if (typeof raw !== "string" || !raw) continue;
        let t = raw;
        while (t.startsWith(prefix)) t = t.slice(prefix.length);
        bare.add(t);
      }
      const policy: Record<string, "allow"> = {};
      for (const t of bare) {
        policy[t] = "allow";
