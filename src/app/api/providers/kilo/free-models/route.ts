...
let cachedModels: Array<{ id: string; name: string; isFree: boolean; context_length: number }> | null = null;
...
export async function GET(): Promise<NextResponse> {
  ...
  try {
    ...
    const allModels: KiloModel[] = json.data || [];
    ...
    if (cachedModels) {
      return NextResponse.json({ models: cachedModels, cached: true, warning: (error as Error).message });
    }

    return NextResponse.json(
      { models: [], error: `Failed to fetch Kilo models: ${(error as Error).message}` },
      { status: 502 }
    );
  }
}
