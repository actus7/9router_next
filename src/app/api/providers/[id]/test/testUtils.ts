import { resolveProbePlan, runProbePlan, type ProbeFetch } from "@/server/llm-gateway/probe/probeCredential";
...
export async function testKiloGatewayConnection(apiKey: unknown, effectiveProxy: ConnectionProxyConfig | null = null): Promise<TestResult> {
  if (typeof apiKey !== "string" || apiKey.trim() === "") {
    return { ok: false, error: "Missing API key", refreshed: false };
  }
  ...
  try {
    ...
    return { ok: false, error: (err as Error).message, refreshed: false };
  }
}
...
async function probeBearerGet(url: string, apiKey: string, proxy: ConnectionProxyConfig | null, errorMsg = "Invalid API key", requireSafeDestination = false): Promise<TestResult> {
  ...
  return { ok: res.ok, error: res.ok ? null : errorMsg, refreshed: false };
}
...
async function probeChatCompletions(
  ...
  const ok = !rejectStatuses.includes(res.status);
  return { ok, error: ok ? null : errorMsg };
}
...
async function probePollinationsKey(apiKey: string, proxy: ConnectionProxyConfig | null): Promise<TestResult> {
  ...
  if (res.ok) return { ok: true, error: null, refreshed: false };
  if (res.status === 402) return { ok: true, error: null, warning: "Pollinations key is valid, but its Pollen balance is exhausted.", refreshed: false };
  if (res.status === 403) return { ok: true, error: null, warning: "Pollinations could not verify this scoped key; it was kept active.", refreshed: false };
  return { ok: false, error: "Invalid Pollinations API key", refreshed: false };
}
...
async function testApiKeyConnection(connection: Record<string, unknown>, effectiveProxy: ConnectionProxyConfig | null = null): Promise<TestResult> {
  ...
  if (isOpenAICompatibleProvider(provider)) {
    if (!psd.baseUrl) return { ok: false, error: "Missing base URL", refreshed: false };
    ...
    catch (err) { return { ok: false, error: (err as Error).message, refreshed: false }; }
  }
  ...
  if (isAnthropicCompatibleProvider(provider)) {
    ...
    try {
      ...
      return { ok, error: ok ? null : "Invalid API key or base URL" };
    } catch (err) { return { ok: false, error: (err as Error).message, refreshed: false }; }
  }

  try {
    // The registry-driven engine answers for every provider with a declared
    // plan, which replaced four lookup tables and most of the switch below.
    const plan = resolveProbePlan(provider);
    if (plan) {
      const send: ProbeFetch = (url, init) => plan.direct
        ? fetch(url, init)
        : fetchWithConnectionProxy(url, init, effectiveProxy);
      return await runProbePlan(provider, plan, apiKey, send);
    }
    ...
    switch (provider) {
      ...
      case "cloudflare-ai": {
        if (!psd.accountId) return { ok: false, error: "Missing Account ID", refreshed: false };
      ...
      case "gemini": return fetchWithConnectionProxy(`https://generativelanguage.googleapis.com/v1/models?key=${apiKey}`, {}, effectiveProxy)
        .then(r => ({ ok: r.ok, error: r.ok ? null : "Invalid API key", refreshed: false }));
      ...
      case "qoder": {
        ...
        return { ok: exRes.ok, error: exRes.ok ? null : "Invalid Personal Access Token" };
      }
      ...
      case "naga-ac": case "naga": return fetchWithConnectionProxy(
        ...
        .then(r => ({ ok: r.ok, error: r.ok ? null : "Invalid API key or base URL", refreshed: false }));
...
async function testGenericOpenAiCompatibleConnection(connection: Record<string, unknown>, effectiveProxy: ConnectionProxyConfig | null = null): Promise<TestResult> {
  ...
  if (!cfg || cfg.format !== "openai" || !cfg.baseUrl) {
    return { ok: false, error: "Provider test not supported", refreshed: false };
  }
  if (cfg.noAuth) {
  ...
  try {
    ...
    if (probeRes.status === 401 || probeRes.status === 403) {
      return { ok: false, error: "Invalid API key", refreshed: false };
    }
    if (probeRes.ok) return { ok: true, error: null, refreshed: false };
  ...
  const ok = chatRes.status !== 401 && chatRes.status !== 403;
  return { ok, error: ok ? null : "Invalid API key", refreshed: false };
}
 ...
 */
export async function testNoAuthProvider(providerId: string): Promise<{ ok: boolean; error: string | null; latencyMs: number }> {
  const config = PROVIDERS[providerId];
  if (!config) return { ok: false, error: "Provider not configured in gateway", latencyMs: 0 };
  ...
  return { ok: false, error: "No testable HTTP endpoint", latencyMs: 0 };
}
...
export async function testSingleConnection(id: string): Promise<SingleTestResult> {
  ...
  if (!connection) return { ok: false, error: "Connection not found", refreshed: false, latencyMs: 0, testedAt: new Date().toISOString() };
  ...
  if (effectiveProxy.connectionProxyEnabled && effectiveProxy.connectionProxyUrl && !effectiveProxy.vercelRelayUrl) {
    ...
    if (!proxyResult.ok) {
      ...
      return { ok: false, error: proxyError, latencyMs: 0, testedAt: new Date().toISOString() , refreshed: false };
    }
  }
  ...
  const softWarning = result.ok && (result.warning || result.error);
  const updateData: Record<string, unknown> = {
    testStatus: result.ok ? "active" : "error",
    lastError: result.ok ? (softWarning || null) : result.error,
    lastErrorAt: result.ok
      ? softWarning
        ? new Date().toISOString()
        : null
      : new Date().toISOString(),
    // A proven-working credential must also clear the dispatcher's penalty.
    // Leaving these behind showed the operator "active" while account
    // selection still had the connection backed off. The failing path leaves
    // them alone: the runtime owns them, and a failed probe is not a backoff.
    ...(result.ok ? { errorCode: null, backoffLevel: 0 } : {}),
  };
  ...
  return { ok: result.ok, error: result.error, refreshed: !!result.refreshed, latencyMs, testedAt: new Date().toISOString() };
}
