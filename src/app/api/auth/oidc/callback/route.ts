...
function clearOidcCookies(cookieStore: ReadonlyRequestCookies): void {
...
export async function GET(request: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    clearOidcCookies(cookieStore);
    return NextResponse.redirect(new URL(`/login?error=${encodeURIComponent((error as Error).message || "oidc_callback_failed")}`, getPublicOrigin(request)));
  }
}
