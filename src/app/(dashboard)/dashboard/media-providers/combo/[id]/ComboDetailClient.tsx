...
interface ComboDetailClientProps {
  ...
  initialAliases: Record<string, unknown>;
  initialLogs: unknown[];
}
...
export default function ComboDetailClient({
  ...
  const [providers, setProviders] = useState<string[]>(initialCombo?.models || []);
  const [roundRobin, setRoundRobin] = useState(
    (initialSettings as Record<string, unknown>)?.comboStrategies &&
    typeof (initialSettings as Record<string, unknown>).comboStrategies === "object"
      ? ((initialSettings as Record<string, Record<string, unknown>>).comboStrategies as Record<string, Record<string, unknown>>)?.[initialCombo?.name || ""]?.fallbackStrategy === "round-robin"
      : false
  );
  const [showPicker, setShowPicker] = useState(false);
  const [logs] = useState<string[]>(
    (initialLogs || []).filter((l: unknown) => typeof l === "string" && l.includes(initialCombo?.name || "")).slice(0, 50) as string[]
  );
  ...
  const saveCombo = async (patch: Record<string, unknown>) => {
  ...
  const handleSaveName = async () => {
    ...
    if (ok) {
      // Re-fetch combo after name change
      const res = await fetch(`/api/combos/${comboId}`, { cache: "no-store" });
      if (res.ok) {
  ...
  if (!combo) {
    return <div className="text-text-muted text-sm">Combo not found.</div>;
  }

  const kindLabel = KIND_LABELS[combo.kind || ""] || MEDIA_PROVIDER_KINDS.find((k: { id: string }) => k.id === combo.kind)?.label || "Combo";
