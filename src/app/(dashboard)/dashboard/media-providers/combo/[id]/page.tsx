  ...
  const fetchAll = async () => {
    try {
      ...
      if (keysRes.ok) {
        ...
        setApiKey((k.keys || []).find((x: Record<string, unknown>) => x.isActive !== false)?.key as string || "");
      }
      ...
      setLogs(allLogs.filter((l: unknown) => typeof l === "string" && l.includes(c.name)).slice(0, 50));
    } catch { /* noop */ }
    setLoading(false);
  };
