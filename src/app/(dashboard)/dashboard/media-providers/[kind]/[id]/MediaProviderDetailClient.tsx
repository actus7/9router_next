  ...
  return (
    <div className="flex flex-col gap-8">
      ...
      {!isCustom && provider?.kindNotice?.[kind as string] && (
        <div className="flex items-start gap-3 px-4 py-3 rounded-lg bg-warning border border-warning-border text-warning-foreground dark:text-warning-foreground">
      ...
      {!isCustom && provider?.notice?.text && !provider?.deprecated && (
        <div className="flex flex-col gap-2 rounded-lg border border-info-border bg-info px-3 py-2 sm:flex-row sm:items-center">
          <Info className="size-4" />
          <p className="min-w-0 flex-1 text-xs leading-relaxed text-info-foreground dark:text-info-foreground">{provider.notice!.text}</p>
          {provider.notice?.apiKeyUrl && (
            <a
              href={provider.notice.apiKeyUrl}
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex justify-center rounded bg-info px-2 py-1 text-xs font-medium text-white transition-colors hover:bg-info sm:py-0.5"
            >
              Get API Key →
            </a>
          )}
