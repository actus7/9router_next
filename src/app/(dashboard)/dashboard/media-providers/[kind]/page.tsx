...
export default function MediaProviderKindPage() {
  ...
  useEffect(() => {
    ...
    if (isEmbedding) {
      fetch("/api/provider-nodes", { cache: "no-store" })
        .then((r) => r.json())
        .then((d) => setCustomNodes((d.nodes || []).filter((n: ProviderNode) => n.type === "custom-embedding")))
        .catch(() => {});
    }
    if (supportsCombo) {
  ...
  const customProviders: Provider[] = customNodes.map((n) => ({
    id: n.id,
  ...
  const handleCreateCombo = async () => {
    const base = COMBO_BASE_NAMES[kind as string] || `${kind}-combo`;
    let name = base;
    let i = 1;
