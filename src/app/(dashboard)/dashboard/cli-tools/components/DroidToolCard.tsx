                ...
                <div className="flex flex-col gap-0.5 mb-1">
                  {modelList.map((id) => (
                    <div key={id} className="flex items-center gap-1.5 px-2 py-1 bg-bg-secondary rounded border border-border">
                      ...
                      <Button variant="ghost" size="sm" onClick={() => removeModel(id)} className="text-text-muted hover:text-destructive-foreground shrink-0 p-0 h-auto" title="Remove" aria-label={`Remove model ${id}`}>
                        <X className="size-3" />
                      </Button>
                    </div>
                  ))}
