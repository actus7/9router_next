...
function ModelField({ label, value, placeholder, onChange, onSelect, disabled, help }: ModelFieldProps) {
  return (
...
export default function GrokBuildToolCard({
  tool, isExpanded, onToggle, hasActiveProviders, apiKeys, activeProviders, cloudEnabled, initialStatus,
  ...
  const [grokStatus, setGrokStatus] = useState<StatusData | null>(initialStatus || null);
  ...
  const hydrateForm = useCallback((status: StatusData) => {
