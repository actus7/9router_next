          ...
          {(openclawStatus?.agents || []).filter((a: { agentDir?: string }) => a.agentDir).map((agent: { id: string; name?: string }) => (
            <div key={agent.id} className="flex items-center gap-2 pl-4">
              ...
              <div className="relative w-full min-w-0">
                ...
                {agentModels[agent.id] && <Button variant="ghost" size="sm" onClick={() => setAgentModels(prev => ({ ...prev, [agent.id]: "" }))} className="absolute right-1 top-1/2 -translate-y-1/2 p-0.5 text-text-muted hover:text-destructive-foreground" title="Clear" aria-label="Clear model selection"><X className="size-4" /></Button>}
              </div>
