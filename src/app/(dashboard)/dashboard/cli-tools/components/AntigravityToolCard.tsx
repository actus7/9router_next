...
interface ToolInfo {
  name: string;
  description?: string;
  defaultModels: Array<{ alias: string; name: string; envKey?: string; defaultValue?: string }>;
}
...
export default function AntigravityToolCard({
  ...
  const [status, setStatus] = useState<StatusData | null>((initialStatus as StatusData) || null);
  ...
  useEffect(() => {
    if (initialStatus) setStatus(initialStatus as StatusData);
  }, [initialStatus]);
