...
const EMPTY_CAPACITY_ADAPTER: Record<string, CapEntry> = {
...
export default function CombosPage() {
  ...
  const [comboStrategies, setComboStrategies] = useState<Record<string, Strategy>>({});
  const [capacityAdapter, setCapacityAdapter] = useState<Record<string, CapEntry>>(EMPTY_CAPACITY_ADAPTER);
  ...
  const fetchData = async () => {
    try {
      ...
      if (combosRes.ok) setCombos((combosData.combos || []).filter((c: Combo) => !c.kind || c.kind === "llm"));
      if (providersRes.ok) {
        ...
        <div className="flex w-full flex-col gap-2 sm:w-auto sm:flex-row sm:items-center sm:gap-3 sm:shrink-0">
          ...
          <div className="w-full sm:w-[200px]">
            <Select
              ...
              value={current}
              onChange={(e: React.ChangeEvent<HTMLSelectElement>) => onSetStrategy({ fallbackStrategy: e.target.value })}
  ...
  return (
    ...
    >
      ...
      {editing ? (
        ...
        <div
          className="min-w-0 flex-1 cursor-text truncate rounded px-1.5 py-0.5 font-mono text-xs text-text-main hover:bg-surface-2/50"
      ...
      <div className="flex shrink-0 items-center gap-0.5">
        <button
          onClick={onMoveUp}
          disabled={isFirst}
          className={`p-0.5 rounded ${isFirst ? "text-text-muted/20 cursor-not-allowed" : "text-text-muted hover:text-primary hover:bg-surface-2/50"}`}
          title="Move up"
        >
          <span className="material-symbols-outlined text-[12px]">arrow_upward</span>
        </button>
        <button
          onClick={onMoveDown}
          disabled={isLast}
          className={`p-0.5 rounded ${isLast ? "text-text-muted/20 cursor-not-allowed" : "text-text-muted hover:text-primary hover:bg-surface-2/50"}`}
          title="Move down"
        >
          <span className="material-symbols-outlined text-[12px]">arrow_downward</span>
        </button>
