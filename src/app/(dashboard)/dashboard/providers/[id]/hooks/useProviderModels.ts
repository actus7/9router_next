...
export function useProviderModels({
  ...
  const discoveryHook = useModelDiscovery({
    ...
    staticModels,
    catalogCleared,
    customModels: customHook.customModels,
    modelAliases: customHook.modelAliases,
    disabledModelIds: disabledHook.disabledModelIds,
    onAddCustomModel: customHook.handleAddCustomModel,
    onFetchDisabledModels: disabledHook.fetchDisabledModels,
    onClearTestResults: () => setModelTestResultsRef.current({}),
    setCustomModels: customHook.setCustomModels,
    setModelAliases: customHook.setModelAliases,
    setDisabledModelIds: disabledHook.setDisabledModelIds,
  });
  ...
  const testingHook = useModelTesting({
    ...
    providerId,
    models,
    kiloFreeModels: discoveryHook.kiloFreeModels,
    disabledModelIds: disabledHook.disabledModelIds,
    onDisableModels: async (ids: string[]) => {
      try {
        ...
        if (response.ok) {
          disabledHook.setDisabledModelIds((current) => [...new Set([...current, ...ids])]);
          notify.warning(`${ids.length} ${translate("model(s) no longer available upstream — moved to Disabled")}`);
        }
      } catch (error) {
  ...
  const providerThinkingLevels = (() => {
    ...
    for (const m of discoveryHook.kiloFreeModels) addLevels(m.id);
    for (const entry of customHook.customModels) {
  ...
  return {
    modelAliases: customHook.modelAliases,
    customModels: customHook.customModels,
    modelTestResults: testingHook.modelTestResults,
    modelsTestError: testingHook.modelsTestError,
    testingModelIds: testingHook.testingModelIds,
    testAllModels: testingHook.testAllModels,
    setTestAllModels: testingHook.setTestAllModels,
    showAddCustomModel,
    setShowAddCustomModel,
    suggestedModels: discoveryHook.suggestedModels,
    liveModels: discoveryHook.liveModels,
    kiloFreeModels: discoveryHook.kiloFreeModels,
    disabledModelIds: disabledHook.disabledModelIds,
    refreshingModels: discoveryHook.refreshingModels,
    importingQoderModels: discoveryHook.importingQoderModels,
    clearingModels: discoveryHook.clearingModels,
    staticModels,
    models,
    providerThinkingLevels,
    fetchDisabledModels: disabledHook.fetchDisabledModels,
    handleDisableModel: disabledHook.handleDisableModel,
    handleEnableModel: disabledHook.handleEnableModel,
    handleDisableAll: disabledHook.handleDisableAll,
    handleEnableAll: disabledHook.handleEnableAll,
    handleClearProviderModels: discoveryHook.handleClearProviderModels,
    fetchAliases: customHook.fetchAliases,
    fetchCustomModels: customHook.fetchCustomModels,
    handleDeleteAlias: customHook.handleDeleteAlias,
    handleAddCustomModel: customHook.handleAddCustomModel,
    handleDeleteCustomModel: customHook.handleDeleteCustomModel,
    handleImportQoderModels: discoveryHook.handleImportQoderModels,
    handleRefreshModels: discoveryHook.handleRefreshModels,
    handleTestAllModels: testingHook.handleTestAllModels,
    handleCancelTestAllModels: testingHook.handleCancelTestAllModels,
    handleTestModel: testingHook.handleTestModel,
  };
}
