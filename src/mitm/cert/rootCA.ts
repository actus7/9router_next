...
const ROOT_CA_KEY_PATH: string = path.join(MITM_DIR, "rootCA.key");
const ROOT_CA_CERT_PATH: string = path.join(MITM_DIR, "rootCA.crt");

interface RootCAPaths {
  key: string;
  cert: string;
}

interface RootCAKeys {
...
function isCertExpired(certPath: string): boolean {
  try {
    const cert: forge.pki.Certificate = forge.pki.certificateFromPem(fs.readFileSync(certPath, "utf8"));
    const expiryThreshold: Date = new Date(Date.now() + 30 * 24 * 60 * 60 * 1000);
 ...
 */
function loadRootCA(): RootCAKeys {
  if (!fs.existsSync(ROOT_CA_KEY_PATH) || !fs.existsSync(ROOT_CA_CERT_PATH)) {
    throw new Error("Root CA not found. Generate it first.");
  }

  const keyPem: string = fs.readFileSync(ROOT_CA_KEY_PATH, "utf8");
  const certPem: string = fs.readFileSync(ROOT_CA_CERT_PATH, "utf8");
 ...
 */
function generateLeafCert(domain: string, rootCA: RootCAKeys): LeafCert {
