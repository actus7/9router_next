 ...
 */
function isChatRequest(tool: string, req: IncomingMessage): boolean {
  const patterns: string[] = URL_PATTERNS[tool] || [];
  if (patterns.some((p: string) => (req.url || "").includes(p))) return true;
  if (tool === "kiro") {
    const target: string = String(req.headers?.["x-amz-target"] || "");
...
const MODEL_PATTERNS: Record<string, ModelPattern[]> = {
  antigravity: [
...
const LOG_BLACKLIST_URL_PARTS: string[] = [
...
type ToolName = "antigravity" | "copilot" | "kiro" | "cursor" | null;

function getToolForHost(host: string | undefined): ToolName {
...
function isBinaryData(buffer: Buffer | null | undefined): boolean {
  if (!buffer || buffer.length === 0) return false;
  const sample: Buffer = buffer.slice(0, Math.min(100, buffer.length));
  let nonPrintable: number = 0;
  for (let i = 0; i < sample.length; i++) {
    const byte: number = sample[i];
...
function extractModel(url: string, body: Buffer): string | null {
  const urlMatch: RegExpMatchArray | null = url.match(/\/models\/([^/:]+)/);
  const urlModel: string | null = urlMatch?.[1] || null;

  if (isBinaryData(body)) return urlModel;

  try {
    const parsed: any = JSON.parse(body.toString());
    if (parsed.conversationState) {
      return parsed.conversationState.currentMessage?.userInputMessage?.modelId || null;
    }
    const model: string | null = urlModel || parsed.model || null;
    const cleanModelName: string = String(model).replace(/^models\//, "");
    if (cleanModelName === "gemini-3.6-flash-tiered" || cleanModelName === "gemini-3.7-flash-tiered") {
      const ver: string = cleanModelName.includes("3.7") ? "3.7" : "3.6";
      const rawLevel: string | undefined = parsed.request?.generationConfig?.thinkingConfig?.thinkingLevel
        || parsed.generationConfig?.thinkingConfig?.thinkingLevel;
      const level: string = ["high", "medium", "low"].includes(String(rawLevel).toLowerCase())
